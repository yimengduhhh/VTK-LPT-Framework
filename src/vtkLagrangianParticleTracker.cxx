  ...
  vtkInformationVector** inputVector, vtkInformationVector* outputVector)
{
  ...
  this->IntegrationModel->SetModelTimeStep(this->GetOneStepTime());
  ...
  vtkPointData* seedData, int nVar, std::queue<vtkLagrangianParticle*>& particles)
{
  ...
  for (vtkIdType i = 0; i < seeds->GetNumberOfPoints(); i++)
  {
    ...
    MyParticle(particle, seeds, seedData, i);
    vtkDataArray* DiameterArray =
      vtkDataArray::SafeDownCast(this->IntegrationModel->GetSeedArray(6, seedData));
    double diameter = DiameterArray->GetTuple1(i);
    double radius = diameter / 2.0;
    particle->SetRadius(radius);

    particle->SetFlowId(1);
    particle->SetMultiLoopId(1);
  ...
  vtkPolyData* particlePathsOutput, vtkPolyLine* particlePath, vtkDataObject* interactionOutput)
{
  ...
  int singleFlowParticleNum = 0;

  int TotalMaxNumber = this->MaximumNumberOfSteps * this->FileListNumber;

  int LocalFlowId = 1;

  if (this->IfUseMultiLoop)
  {
    TotalMaxNumber = TotalMaxNumber * this->MultiLoopNumber;
  }
  ...
  while (particle->GetTermination() == vtkLagrangianParticle::PARTICLE_TERMINATION_NOT_TERMINATED)
  {
    ...
    double stepTime = this->GetOneStepTime();
    double stepTimeMin = 0.0000005;
    double stepTimeMax = 0.0000015;

    // this->IntegrationModel->IfParticleNearSurface(particle);
    //  Integrate one step
    ...
    if (!stagnating)
    {
      ...
      if (this->IntegrationModel->GetIfConsiderRadius() ==
        vtkLagrangianBasicIntegrationModel::EFFECT_OPEN)
      {
        this->IntegrationModel->BounceRadius2(particle);
      }
    ...
    if (singleFlowParticleNum == (this->MaximumNumberOfSteps - 1))
    {
      particle->MoveToNextFlow();
      singleFlowParticleNum = 0;
      if (this->IfUseMultiLoop)
      {
        LocalFlowId = LocalFlowId + 1;
        if (LocalFlowId == (this->FileListNumber + 1))
        {
          particle->ResetFlowId();
          particle->MoveToNextLoop();
          LocalFlowId = 1;
        }
      }
    }
    ...
    {
      singleFlowParticleNum = singleFlowParticleNum + 1;
    }

    if (this->MaximumNumberOfSteps > -1 && particle->GetNumberOfSteps() == TotalMaxNumber &&
...
void vtkLagrangianParticleTracker::AddFlowFile()
{
  if (this->FileListNumber > 1)
  {
    int fileindex;
    int ProcessFileId = 1;
    std::string ProcessFileIdstr;
    std::string FileNameProfix;
    std::string NextFileName;
    std::string FileNameStr;
    FileNameStr = this->GetFileName();
    fileindex = FileNameStr.find_last_of(".");
    FileNameProfix = FileNameStr.substr(0, fileindex - 1);
    vtkNew<vtkXMLUnstructuredGridReader> FlowReader;

    while (ProcessFileId < this->FileListNumber)
    {
      int AnotherId = ProcessFileId + 1;
      ProcessFileIdstr = std::to_string(AnotherId);
      NextFileName = FileNameProfix;
      NextFileName.append(ProcessFileIdstr);
      NextFileName.append(".vtu");
      FlowReader->SetFileName(NextFileName.c_str());
      FlowReader->Update();
      vtkDataObject* NextFlowData = FlowReader->GetOutputDataObject(0);
      std::cout << "read file done";
      if (!this->AddFlowData(NextFlowData))
      {
        vtkErrorMacro(<< "Cound not Add the FlowData,Please check");
      }
      std::cout << "add data done";
      ProcessFileId = ProcessFileId + 1;
    }
  }
}

bool vtkLagrangianParticleTracker::AddFlowData(vtkDataObject* flow)
{
  // Check flow dataset type
  vtkCompositeDataSet* hdInput = vtkCompositeDataSet::SafeDownCast(flow);
  vtkDataSet* dsInput = vtkDataSet::SafeDownCast(flow);
  if (hdInput)
  {
    // Composite data
    vtkSmartPointer<vtkCompositeDataIterator> iter;
  ...
  {
    vtkErrorMacro(<< "This filter cannot handle changing flow of type: "
                  << (flow ? flow->GetClassName() : "(none)"));
    return false;
  }
  return true;
}

void vtkLagrangianParticleTracker::MyParticle(
  vtkLagrangianParticle* particle, vtkDataSet* seeds, vtkPointData* seedData, int index)
{
  if (particle == nullptr)
  {
    std::cout << "error particle" << endl;
    return;
  }
  vtkDataArray* tracktime = nullptr;
  if (!seeds)
  {
    vtkErrorMacro(<< "Cannot generate Particles without seeds");
    return;
  }
  tracktime = vtkDataArray::SafeDownCast(this->IntegrationModel->GetSeedArray(24, seedData));
  double particle_time = 0.0;
  particle_time = tracktime->GetTuple1(index);

  if (particle_time > 0.5)
  {
    particle_time = 0.4;
  }
  double delt_time = this->GetOneStepTime();
  vtkIdType particle_step = (vtkIdType)(particle_time / delt_time);
  particle->SetNumberOfSteps(particle_step);
}
